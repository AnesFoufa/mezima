...
import Interpreter (EvaluationError (..), Evaluator (runEvaluator), evaluator, initSymbolsTable, runEvaluator)
import RIO
import SExp
import Test.Hspec
...
spec :: Spec
spec = do
    let evaluates sexp = evalState (runEvaluator evaluator sexp) initSymbolsTable
    ...
    describe "evaluator basics" do
        ...
        prop "evaluates S Integer as S Integer" do
            \i -> evaluates (SInteger i) `shouldBe` Right (if i >= 0 then SInteger i else SSExp [minusIdentifier, SInteger (-i)])
        prop "evaluates S Double as S Double" do
            \i -> evaluates (SDouble i) `shouldBe` Right (if i >= 0 then SDouble i else SSExp [minusIdentifier, SDouble (-i)])
        prop "evaluates S String as S String" do
            \i -> evaluates (SString i) `shouldBe` Right (SString i)
        it "Evaluates list prefixed as lists" do
            evaluates (SSExp [listIdentifier]) `shouldBe` Right (SSExp [listIdentifier])
    ...
    describe "evaluator numerics" do
        ...
        it "Evaluates nested sums" do
            evaluates (SSExp [sumIdentifier, SSExp [sumIdentifier, SInteger 1, SInteger 2], SInteger 3, SInteger 4]) `shouldBe` Right (SInteger 10)
        it "Evaluates equality without arguments as True" do
            evaluates (SSExp [equalityIdentifier]) `shouldBe` Right (SBool True)
        prop "Evaluates equality of similar integers as True" do
            \(i, n) -> evaluates (SSExp (equalityIdentifier : replicate i (SInteger n))) `shouldBe` Right (SBool True)
        prop "Evaluates equality of similar doubles as True" do
            \(i, d) -> evaluates (SSExp (equalityIdentifier : replicate i (SDouble d))) `shouldBe` Right (SBool True)
        it "Evaluates equal integers and and doubles as True" do
            evaluates (SSExp [equalityIdentifier, SDouble 1.0, SInteger 1]) `shouldBe` Right (SBool True)
        ...
        it "Evaluates negative integers" do
            evaluates (SSExp [minusIdentifier, SInteger 42]) `shouldBe` Right (SSExp [minusIdentifier, SInteger 42])
        it "Evaluates negative doubles" do
            evaluates (SSExp [minusIdentifier, SDouble 42.5]) `shouldBe` Right (SSExp [minusIdentifier, SDouble 42.5])
        ...
        it "Evaluates inversion of integers" do
            evaluates (SSExp [divisionIdentifier, SInteger 2]) `shouldBe` Right (SDouble 0.5)
        it "Evaluates inversion of doubles" do
            evaluates (SSExp [divisionIdentifier, SDouble 0.5]) `shouldBe` Right (SDouble 2.0)
        it "Evaluates division of integers" do
            evaluates (SSExp [divisionIdentifier, SInteger 2, SInteger 4]) `shouldBe` Right (SDouble 0.5)
        it "Evaluates division of doubles" do
            evaluates (SSExp [divisionIdentifier, SDouble 2.0, SDouble 0.5]) `shouldBe` Right (SDouble 4.0)
        it "Evaluates division of a double and an integer" do
            evaluates (SSExp [divisionIdentifier, SDouble 2.0, SInteger 1]) `shouldBe` Right (SDouble 2.0)
        it "Evaluates division of a integer and a double" do
            evaluates (SSExp [divisionIdentifier, SInteger 1, SDouble 0.5]) `shouldBe` Right (SDouble 2.0)
